...
class MarkOS:
    ...
    def add_app_to_taskbar(self, app_name, close_command=None, app_id=None):
    ...
    def close_app(self, app_id):
        ...
        if app_id in self.open_apps:
            ...
            # Ejecutar función de cierre si existe
            if app_data["close_fn"]:
                app_data["close_fn"]()
    ...
    def open_calc(self):
        try:
            calc.calc()
            self.add_app_to_taskbar("Calculadora")
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo abrir la calculadora: {str(e)}")
    
    def open_browser(self):
        browser_id = f"navegador_{time.time()}"
        
        def close_browser():
            # Esta función encontrará y cerrará la ventana correcta
            if browser_id in self.open_apps:
                browser_data = self.open_apps[browser_id]
                if "window" in browser_data:
                    try:
                        browser_data["window"].close()
                    except:
                        pass
                self.close_app(browser_id)
        
        # Añadir a la barra de tareas primero
        self.add_app_to_taskbar("Navegador", close_browser, browser_id)
        
        # Lanzar el navegador directamente (sin hilos)
        try:
            browser_window = webs.launch_browser(
                self.root,
                lambda: self.root.after(100, close_browser)
            )
            # Guardar referencia a la ventana del navegador
            if browser_id in self.open_apps:
                self.open_apps[browser_id]["window"] = browser_window
        except Exception as e:
